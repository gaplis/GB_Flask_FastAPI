...
DATABASE_URL = "sqlite:///Seminars/Seminar_6/task_3.db"
...
async def create_task(user: TaskIn):
    query = tasks.insert().values(**user.model_dump())
...
async def update_task(task_id: int, new_user: TaskIn):
    query = tasks.update().where(tasks.c.id == task_id).values(**new_user.model_dump())
    await database.execute(query)
    return {**new_user.model_dump(), "id": task_id}
